...
function loadIds() {
  checkBoxes.forEach(element => {
    let generateId = content =>
      content.toLocaleLowerCase().replace(/[ ,.!?;:'-]/g, "");
...
function loadValues() {
  let initialCounterValue =
    localStorage.getItem("checkedBoxesCountFromStorage") || 0,
    initialCheckedCounterValue =
      localStorage.getItem("checkedBoxesCountFromStorage") || 0;

  checkBoxesCount.textContent = checkBoxesLength;
  checkedBoxesCount.textContent = initialCounterValue;
  progress.style.transform = `scaleX(${initialCheckedCounterValue /
    checkBoxesLength})`;
...
function updateCounter() {
  ...
  progress.style.transform = `scaleX(${globalCheckedCounter /
    checkBoxesLength})`;
